...
function WeatherDisplay(props) {
  ...
  return (
    <div className="weather-display col-12 col-md-6 mx-auto" >
     ...
     <div className='location'> <h2 style={{color:props.mode==='dark'?'white':'#042743' }}>{name}</h2></div>
      <img className='countryimage ms-3' src={`https://flagcdn.com/144x108/${country}.png `} alt={`Flag of ${sys?.country}`} />
    
      <div className='description mt-2'> <h3 style={{color:props.mode==='dark'?'white':'#042743' }}>{weather[0].description}</h3></div>
      <div className='description mt-2'> <p style={{color:props.mode==='dark'?'white':'#042743' }}>{currentDate}</p></div>
      <div className='description mt-0'><img src={iconUrl}/></div>
      
      <div className='description mt-2'> <h1 style={{color:props.mode==='dark'?'white':'#042743' }} >{main.temp}°C</h1></div>
